...
<form method="POST" action="{{ route('register') }}">
 ...
 <input type="hidden" name="auth_attempt" value="register">
    ...
    <p>   
     ...
     <p>   
        <label>Passwords <span>*</span></label>
        <input id="password" type="password" pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}" class="form-control @error('password') @if(Session::get('last_auth_attempt')=='register') is-invalid @endif @enderror" name="password" title="Must contain at least one number and one uppercase and lowercase letter, and at
