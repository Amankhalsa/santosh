...
<img src="{{asset('uploaded_files/product/'.$product->category_image_name)}}" alt="image of {{$brand->category_name}}">
@else
...
@endphp        
<li>
<a href="{{url('/frame/'.$product->category_slug_name.'.html')}}"  class="colorCol actColor btn-tool @if($group->category_slug_name==$product->category_slug_name) color-btn @endif">
...
@else
<li>
<a href="{{url('/frame/'.$product->category_slug_name.'.html')}}" target="_blank"  class="colorCol" >
...
@endif
       ...
       <a href="{{url('/frame/'.$product['category_slug_name'].'.html')}}" class="btn btnDark w-100 addCartBtn">ADD TO CART</a>
            ...
            <div class="btnCol text-center">
              <a href="{{url('/eyeglasses/men')}}" class="btn btnPrimary minWdBtn btnNew">Shop Now</a>
            </div>
          </div>
          @endif