...
<span style="padding:8px;font-size:16px;box-shadow:2px 2px 2px grey;" class="badge badge-pill badge-secondary"><b>Total:</b> {{ $users->total() }}</span>
...
@if($users->isNotEmpty())
 ...
 <form action="{{ route('bottom-button-action-users') }}" method="post" onsubmit="return checkboxValidation()">
  @csrf
  @method('POST')
  <div class="row">
     ...
     @foreach($users as $user_data)
        <tr>
        <td class="text-center v-align"><input type="checkbox" name="user_ids[]" id="ids[]" class="user_ids" value="{{ $user_data->id }}"/> {{ $i++ }}</td>
        
        <td class="text-center v-align">{{ $user_data->name }}</td>
        <td class="text-center v-align" >{{ $user_data->email }}</td>
        <td class="text-center v-align" >{{ $user_data->mobile }}</td>
        <td class="text-center v-align">
        @if($user_data->status=="Active")
        <span class="badge badge-success">{{ $user_data->status }}</span>
        @else
        <span class="badge badge-danger">{{ $user_data->status }}</span>
        @endif
        </td>

        <td class="text-center v-align">
        <a title="Edit User" href="{{ route('edit-user', $user_data->id) }}" data-toggle="tooltip"><i class="fa fa-edit"></i></a>
