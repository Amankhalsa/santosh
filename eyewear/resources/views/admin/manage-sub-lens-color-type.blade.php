...
@section('title','Manage Sub Lens Color Type')
...
<span><a href="{{ route('manage-lens-color-type') }}" class="btn btn-info" style="font-size:15px"><i class="fa fa-angle-double-left"></i> Back</a></span>
&nbsp;&nbsp;


<span><a href="{{ route('add-sub-lens-color-type-form', $category_parent_id) }}" class="btn btn-primary"> <i class="fa fa-plus"></i> Add</a></span>
...
@endif
 ...
 <form action="{{ route('bottom-button-action-sub-lens-color-type', $category_parent_id) }}" method="post" onsubmit="return checkboxValidation()">
  @csrf
  @method('PUT')
  ...
  <div class="row">
     ...
     @foreach($subcategories as $subcategory_data)
        ...
        <a href="{{route('manage-tint-color',[$category_parent_id,$subcategory_data->id])}}">{{ $subcategory_data->category_name }} <i class="fas fa-arrow-right"></i></a> 
        </td>

        <td class="text-center v-align">
        {{ $subcategory_data->category_price }} 
        </td>
        ...
        <td class="text-center v-align">
        <a title="Edit sub lens color type" href="{{ route('edit-sub-lens-color-type', [$category_parent_id, $subcategory_data->id]) }}" data-toggle="tooltip"><i class="fa fa-edit"></i></a>
