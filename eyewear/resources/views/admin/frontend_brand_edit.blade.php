...
@endif
 ...
 <div class="container-fluid">
  ...
  <div class="row">
        <div class="col-lg-12">
<form method="post" action="{{route('manage.brand_update',$data->id )}}" enctype="multipart/form-data">
    @csrf
...
<h3 class="text-center"> Edit brand </h3>
 ...
 <!--select options -->
  ...
  @php
$brands = DB::table('categories')->orderBy('category_name')->select(['id','category_name','category_slug_name'])->where('category_status','Active')->where('category_parent_id','0')->get();
@endphp
...
<!--========== select url =============-->
  ...
  <!--name -->
    <div class="form-group">
    <input type="text" class="form-control" id="brandname" value="{{$data->name}}"  disabled >
  </div>
  <!--name-->
    ...
    <input type="url" class="form-control" id="brandurl" value="{{$data->url}}" disabled>
  </div>
  <!--url-->
  ...
  <!------------------- image -------------------->
  	<div class="form-group">
			<div class="controls">
			<img src="{{asset($data->image)}}" width="100" height="100" id="output">
			</div>
	</div>
	<!-------------- image ---------------------->
  <div class="form-group">
