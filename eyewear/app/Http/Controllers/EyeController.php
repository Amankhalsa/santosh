...
// ================ eyeglasses ===============
public function eyeglasses(Request $request){
         $ip = \Request::ip();
 	//$ip = '1.32.239.255';
 	$location = \Location::get($ip);
    ...
    // Get Brands-
     ...
     // Get SunGlasses
     $sunglasses_count = Category::where('category_frame','Eyeglasses')->where('category_type','Product')->where('category_status','Active')->where('category_qty','>',0)->count();
     ...
     $frames = Category::where('category_status','Active')->where('category_type','Product')->latest()->get();  
 	    ...
 	    if($request->page)
 	    {
 	        $sunglasses = Category::where('category_frame','Eyeglasses')->where('category_type','Product')->where('category_status','Active')->where('category_qty','>',0)->get()->skip($request->page)->take(6);
    return view('eye_glasses_ajax',compact('meta_tag','brands','sunglasses','frames','ip_country','sunglasses_count'));
 	    }
    return view('eye_glasses_new',compact('meta_tag','brands','sunglasses','frames','ip_country','sunglasses_count'));
...
public function sunglasses(){
    ...
    // Get Brands
     ...
     // Get SunGlasses
     $sunglasses = Category::where('category_frame','Sunglasses')->where('category_type','Product')->where('category_status','Active')->where('category_qty','>',0)->get()->take(9);
    ...
    return view('eye_glasses_ajax',compact('meta_tag','brands','sunglasses','frames','ip_country'));
     }
...
// ============================== main_cat end =======================    
     public function gentle_man(){
      ...
      $arr = Category::where('category_for','!=','Woman')->where('category_qty','>',0)->where('category_status','Active')->paginate(39);
...
$frame_colors = ProductColor::all();
    ...
    $products=json_decode($arr,true);
     ...
     public function woman(){
        $ip_country=$this->ip_country;
      $product_for="Woman";    
      $arr = Category::where('category_for','!=','Gentle Man')->where('category_qty','>',0)->where('category_status','Active')->paginate(39);
      $all_brands = Category::where('category_parent_id','0')->where('category_status','Active')->get();
...
return view('product-for',compact('products','product_for','all_brands','top_products','data','frame_types','frame_shapes','frame_materials','ip_country','frame_colors'));
     ...
     public function junior(){
      $ip_country=$this->ip_country;
      $product_for="Junior";    
      $arr = Category::where('category_for','!=','Junior')->where('category_qty','>',0)->where('category_status','Active')->paginate(20);
      $all_brands = Category::where('category_parent_id','0')->where('category_status','Active')->get();
...
return view('product-for',compact('products','product_for','all_brands','top_products','data','frame_types','frame_shapes','frame_materials','ip_country','frame_colors','glass_type'));
  ...
  public function product_detail(Request $request){
    ...
    $related_data = Category::where('category_status','Active')->where('category_type','product')->where('category_frame',$product_data->category_frame)->where('id','!=',$product_data->id)->where('category_qty','>',0)->where('shape',$product_data->shape)->limit(12);
...
return view('category',compact('main_category','products','all_brands','top_products','search_product','min_price','max_price','order_filter','gender_array','shape_array','frame_types','frame_shapes','frame_materials','frame_array','material_array','data','frame_colors','color_array','ip_country'));
    ...
    $arr = Category::where('category_status','Active')->where('category_type','Product')->where('category_for',$product_for)->where('category_qty','>',0);
